...
class Bot {
    constructor(game) {
        ...
        this.currPlayer = game.currPlayer.id
        this.height = game.height
        this.width = game.width

    }

    checkForWin(board, currPlayer, HEIGHT, WIDTH) {
        function _win(cells) {
    ...
    checkForFutureWin(board, currPlayer, HEIGHT, WIDTH) {
        function _win(cells) {
            // Check four cells to see if they're all color of current player
            //  - cells: list of four (y, x) cells
            //  - returns true if all are legal coordinates & all match currPlayer
        ...
        for (var y = 0; y < HEIGHT; y++) {
            for (var x = 0; x < WIDTH; x++) {
                ...
                //get horrizontal
                var horiz = [[y, x], [y, x + 1], [y, x + 2], [y, x + 3]];
                if (this.checkIfBelowIsNotNull(board, horiz) === false) {
                    horiz = null
                }

                //get vertical
                var vert = [[y, x], [y + 1, x], [y + 2, x], [y + 3, x]];
                if (this.checkIfBelowIsNotNull(board, vert) === false) {
                    vert = null
                }

                //get diag
                var diagDR = [[y, x], [y + 1, x + 1], [y + 2, x + 2], [y + 3, x + 3]];
                if (this.checkIfBelowIsNotNull(board, diagDR) === false) {
                    diagDR = null
                }
                ...
                if (this.checkIfBelowIsNotNull(board, diagDL) === false) {
                    diagDL = null
                }
    ...
    checkForPincerFormation(board, currPlayer, HEIGHT, WIDTH) {
        function _win(cells) {
            // Check four cells to see if they're all color of current player
            //  - cells: list of four (y, x) cells
            //  - returns true if all are legal coordinates & all match currPlayer
    ...
    checkIfBelowIsNotNull(board, arrOfCords) {
    ...
    findWinningMove(board, currPlayer) {

        let possibleY = []

        for (let x = 0; x < board[0].length; x++) {
            let y = this.findSpotForCol(board, x, board.length)
            possibleY.push(y)
        ...
        for (let i = 0; i < possibleY.length; i++) {
            ...
            if (possibleY[i] !== null) {
                ...
                let isThisMoveAWin = this.checkForWin(copyBoard, currPlayer, copyBoard.length, copyBoard[0].length)
                if (isThisMoveAWin)
                    winningX.push(i)
    ...
    findFutureWinningMove(board, currPlayer) {
        ...
        for (let i = 0; i < possibleY.length; i++) {
            ...
            if (possibleY[i] !== null) {
                copyBoard[possibleY[i]][i] = currPlayer
                let isThisMoveAWin = this.checkForFutureWin(copyBoard, currPlayer, copyBoard.length, copyBoard[0].length)
                if (isThisMoveAWin)
                    winningX.push(i)
            }
        }
    ...
    findFuturePincerMove(board, currPlayer) {
        ...
        for (let i = 0; i < possibleY.length; i++) {
            ...
            if (possibleY[i] !== null) {
                copyBoard[possibleY[i]][i] = currPlayer
                let isThisMoveAPincer = this.checkForPincerFormation(copyBoard, currPlayer, copyBoard.length, copyBoard[0].length)
    ...
    findPincerDefenseMove(board, currPlayer) {
        ...
        for (let i = 0; i < possibleEnemyY.length; i++) {
            ...
            if (possibleEnemyY[i] !== null) {
                ...
                let isThisMoveAWin = this.checkForPincerFormation(copyBoard, enemyPlayer, copyBoard.length, copyBoard[0].length)
                if (isThisMoveAWin)
                    blockingX.push(i)
            }
        }
    ...
    findFutureDefenseMove(board, currPlayer) {
        ...
        for (let i = 0; i < possibleEnemyY.length; i++) {
            ...
            if (possibleEnemyY[i] !== null) {
                copyBoard[possibleEnemyY[i]][i] = enemyPlayer
                let isThisMoveAWin = this.checkForFutureWin(copyBoard, enemyPlayer, copyBoard.length, copyBoard[0].length)
                if (isThisMoveAWin)
                    blockingX.push(i)
            }
        }
    ...
    findDefenceMove(board, currPlayer) {
        ...
        for (let i = 0; i < possibleEnemyY.length; i++) {
            ...
            if (possibleEnemyY[i] !== null) {
                copyBoard[possibleEnemyY[i]][i] = enemyPlayer
                //console.log(copyBoard)
                let isThisMoveAWinForEnemy = this.checkForWin(copyBoard, enemyPlayer, copyBoard.length, copyBoard[0].length)
    ...
    checkIfMoveWillResultInEnemyWinning(futureWinMove, board) {
        ...
        for (let i = 0; i < futureWinMove.length; i++) {
            ...
            let movesEnemyCanDoToWin = this.findWinningMove(copyBoard, 2)
    ...
    checkIfMoveWillResultInPincerAttack(futureWinMove, board) {
        ...
        for (let i = 0; i < futureWinMove.length; i++) {
            ...
            let movesEnemyCanDoToPincer = this.findFuturePincerMove(copyBoard, 2)
